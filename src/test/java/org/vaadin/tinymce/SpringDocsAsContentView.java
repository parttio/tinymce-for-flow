...
@Route
public class SpringDocsAsContentView extends Div {


    protected TinyMce tinyMce;

    public SpringDocsAsContentView() {
        tinyMce = new TinyMce();

        try {
            String string = IOUtils.toString(getClass().getResource("/sbdocs.html"), Charset.defaultCharset());
            tinyMce.setValue(string);

            if(false) {

                Document.OutputSettings outputSettings = new Document.OutputSettings();
                outputSettings.prettyPrint(false);
                String cleaned = Jsoup.clean(string, "", Safelist.relaxed(), outputSettings);
